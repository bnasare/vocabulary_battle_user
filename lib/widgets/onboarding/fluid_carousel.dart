...
class FluidCarousel extends StatefulWidget {
  ...
  const FluidCarousel(
      {super.key, required this.children, this.initialIndex = 0});
    ...
    return Stack(
      children: [
        GestureDetector(
          key: key,
          onPanDown: (details) {
            // Ignore gestures in the bottom 150px (where button is)
            final size = _getSize();
            if (details.localPosition.dy < size.height - 150) {
              _handlePanDown(details, size);
            }
          },
          onPanUpdate: (details) {
            final size = _getSize();
